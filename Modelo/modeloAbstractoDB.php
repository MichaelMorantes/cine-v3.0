<?php
	abstract class ModeloAbstractoDB {
		...
		protected function foto($query) {			
			try {
				$this->abrir_conexion();
				$sth = $this->conexion->query($query);
				$result=mysqli_fetch_array($sth) or die(mysqli_error());
				$this->cerrar_conexion();
				// echo 'console.log('. $result['picture'] .')';
